...
// 云端获取数据
  Future getCloudData() async {
    ...
    dataModel = model;
    debugPrint('5G Guest3获取的数据:$jsonObj');
    try {
      // List<String> parameterNames = [
      //   'InternetGatewayDevice.WEB_GUI.WiFi.WLANSettings.2.SSIDProfile.4',
      // ];
      // var res = await Request().getACSNode(parameterNames, sn);
      // var data = jsonDecode(res)['data']['InternetGatewayDevice']['WEB_GUI']
      //     ['WiFi']['WLANSettings']['2']['SSIDProfile']['4'];

      var data = model.data![3];
  ...
  setData() async {
    ...
    var parameterNames = {
      ...
      "table": {
        ...
        "value": [
          {
            "id": dataModel!.data![3].id,
            "Enable": dataModel!.data![3].enable,
            "SsidHide": showSsid == false ? "0" : "1",
            "ApIsolate": apVAl == false ? "0" : "1",
            "ShowPasswd": dataModel!.data![3].showPasswd,
            "Is4Guest": dataModel!.data![3].is4Guest,
            "PasswordLength": dataModel!.data![3].passwordLength,
            "PasswordIndex": dataModel!.data![3].passwordIndex,
            "MaxClient": maxVal.text,
            "Ssid": ssidVal.text,
            "Key": password.text,
            "Password1": dataModel!.data![3].password1,
            "Password2": dataModel!.data![3].password2,
            "Password3": dataModel!.data![3].password3,
            "Password4": dataModel!.data![3].password4,
            "Encryption": '$safeVal+$wpaVal',
            "SsidMac": dataModel!.data![3].ssidMac,
            "AllowAccessIntranet": networkCheck == false ? "0" : "1",
            "wifiRadiusServerIP": dataModel!.data![3].wifiRadiusServerIP,
            "wifiRadiusServerPort": dataModel!.data![3].wifiRadiusServerPort,
            "wifiRadiusSharedKey": dataModel!.data![3].wifiRadiusSharedKey
          }
        ]
      }
    };
...
// 提交
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar:
            customAppbar(context: context, title: dataModel != null ? '${dataModel!.data![3].ssid}' : "", actions: <Widget>[
          Container(
